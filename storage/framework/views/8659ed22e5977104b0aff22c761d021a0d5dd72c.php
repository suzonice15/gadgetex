
<div class="container-fluid footer-desktop p-5">
    <div class="row">
        <div class="col-lg-6 col-xl-6">
            <img style="height: 61px;" src="<?php echo e(asset('/images/ICON/logo.svg')); ?>" class="img-fluid">
            <p>GadgetEx continues to provide customers with a reliable and trustworthy online shop along with their two shops, helping to ensure a better and premium business experience.</p>
            <hr>
            <div class="d-flex flex-row">
                Connect with us |  <ul class="socialli">
        <div class="d-flex flex-row justify-content-center ">
                       <a class="solik" href="http://"> <i class="fab fa-facebook-square  social-padding-of-single-product"></i></a>
                        <a class="solik" style="color: #833AB4;" href="http://" target="_blank" rel="noopener noreferrer"> <i class="fab fa-instagram  social-padding-of-single-product"></i></a>
                        <a class="solik" style="color:#25D366;" href="http://" target="_blank" rel="noopener noreferrer"><i class="fab fa-whatsapp  social-padding-of-single-product"></i></a>
                        <a class="solik" style="color:#2867B2;" href="http://" target="_blank" rel="noopener noreferrer"> <i class="fab fa-linkedin  social-padding-of-single-product "></i></a>
                       <a class="solik" style="color:#1DA1F2;" href="http://" target="_blank" rel="noopener noreferrer"> <i class="fab fa-twitter  social-padding-of-single-product"></i></a>
         </div>
        </ul>
            </div>

        </div>

        <div class="col-lg-6 col-xl-6">
            <div class="row">
                <div class="col-lg-4 col-xl-4">
                    <h4 class="ms-4">GadgetEx</h4>
                    <ul style="list-style: none" >
                        <li><a>About Us</a></li>
                        <li><a>Our Mission</a></li>
                        <li><a>Why GadgetEx</a></li>
                        <li><a>Product Quality  </a></li>
                        <li><a>Shop Address  </a></li>
                        <li><a>Contact Us</a></li>
                    </ul>

                </div>

                <div class="col-lg-4 col-xl-4">
                    <h4 class="ms-4">Care</h4>
                    <ul style="list-style: none">
                        <li><a>Take Guide  </a></li>
                        <li><a>Order Tracking  </a></li>
                        <li><a>My Complain    </a></li>

                    </ul>



                </div>
                <div class="col-lg-4 col-xl-4">
                    <h4 class="ms-4">Informations</h4>
                    <ul style="list-style: none">
                        <li><a>Privacy  Policy  </a></li>
                        <li><a>Terms and Conditions    </a></li>
                        <li><a>Return and Replacement      </a></li>
                        <li><a>Warrantly  Policy     </a></li>

                    </ul>

                </div>
            </div>

        </div>
    </div>
<div class="row mt-5">
    <div class="col-sm-6">
        <h4 class="fbytitle">Follow Facebook Page</h4>
        <div class="fbycover">
            <!-- <img class="img-fluid" src="<?php echo e(asset('/images/ICON/fbcover.png')); ?>"> -->

        </div>
    </div>
    <div class="col-sm-6">
    <h4 class="fbytitle">Subscribe YouTube Channel</h4>
    <div class="fbycover">
    <div class="embed-responsive embed-responsive-16by9">
    <iframe width="100%" height="290" src="https://www.youtube.com/embed/BVMsRltq2yU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </div>
        </div>
    </div>
</div>
    <div style="margin-top:20px;" class="row mt5">
        <div class="col-lg-12 col-xl-12 d-flex flex-row justify-content-center" style="background-color:#ddd;border-radius: 10px">

            <h2>Registered Member of </h2>

            <img src="<?php echo e(asset('/images/ICON/Member of eCab.png')); ?>" style="height: 32px;margin-top: 2px;margin-left: 9px;" class="img-fluid">

        </div>
        <div class="col-lg-12 col-xl-12 d-flex flex-row justify-content-center"  style="border: 2px solid #ddd;margin-top: 12px;border-radius: 10px;">



            <img src="<?php echo e(asset('/images/ICON/Payment Partner-01 1.png')); ?>"   class="img-fluid">

        </div>

        <div class="col-lg-12 col-xl-12 d-flex flex-row justify-content-center mt-5"  >

            <h4>@ <?php echo e(date("Y")); ?> .All  Rights Reserved by GadgetEx Developed by <a href="http://it.jibonpata.com" style="text-decoration: none;color:black" target="_blank">Jibonpata IT Limited</a></h4>

        </div>
    </div>
</div>

<div class="container-fluid">
    <img src="<?php echo e(asset('/images/ICON/Capture 21.png')); ?>" class="img-fluid " style="float: left;background: #ddd;/*! margin-right: 7px; */padding: 2px;margin-top: 42px;border-top-right-radius: 42px;margin-left: -11px;height: 82px;">

    <img src="<?php echo e(asset('/images/ICON/Messenger 8.png')); ?>" class="img-fluid " style="float: right;background: #ddd;margin-right: -12px;padding: 12px;margin-top: 50px;border-top-left-radius: 43px;width: 103px;">


</div><?php /**PATH C:\xampp\htdocs\gadgetex\resources\views/fontend/partial/desktop_footer.blade.php ENDPATH**/ ?>